...
| Mode | The PRD holds | The team owns |
|---|---|---|
...
| Mode | Checks |
|------|--------|
| `prd` (default) | dangling `[V#]` markers and V-numbers duplicated inside one layer (LINT-001), unchecked writer-confirmation checkboxes (LINT-002), branch-name citation URLs that aren't commit-pinned (LINT-003), changelog version ordering (LINT-004), leftover `OQ-` items (LINT-005), leftover `> **GUIDE**` blocks (LINT-006), raw analytics event names in ACs and `AE-<n>` rows bound by zero ACs (LINT-007), wire-value leaks into FRs/ACs/Edge Cases (LINT-008), renamed top-level sections and slim PRDs missing their behavioral anchors (LINT-009), unused Product Constants and bare inline bounds in requirements (LINT-010), transport taxonomy in slim-mode Analytics/Support content (LINT-011), wire encodings in slim-mode Semantic Vocabulary Type cells (LINT-012), code wiring (repo paths, source files, route constants) outside Dependencies in slim mode (LINT-013), design-mechanism phrases in the slim-mode behavioral layer — a narrow heuristic backing the reviewer's separation pass (LINT-014) |
...
| `shared-requirements` | migration scan for a consuming project's `docs/shared-requirements.md` — stale obligations from previous framework generations: Localization-section mandates (LINT-201), Technical-Contract-mandatory language that slim mode cannot satisfy (LINT-202), literal-copy mandates (LINT-203). Findings are migration items for the SR owner (fix the SR or record an explicit override), never PRD defects |

The agents call it automatically when the file is present: the writer at Step 4.5 (before saving), the reviewer at step 8.1.2 (PRD violations become Matrix I FAIL rows; review-file violations are fixed in place) and step 8.1.5 (`shared-requirements` mode findings become SR-DRIFT escalations to the SR owner), project-setup at install/upgrade when a `docs/shared-requirements.md` already exists (findings are reported as migration items), and `/create-prd` before Gate 2 (violations surface with the draft notice). If the file is absent, every caller falls back to its manual scans — copying it in is opt-in.

LINT-001 through LINT-010 run in both Technical Contract modes — the mode changes which sections exist, not which rules apply. LINT-001 resolves a `[V#]` marker against the Semantic Vocabulary table, a per-endpoint Vocabulary table, or both (a full-mode PRD repeats each V-number to attach the field binding, so a number present in both layers is expected — two rows for it *inside* one layer is still a defect). LINT-008's source is the optional `API Field` column, so a slim PRD gives it nothing to scan and it skips cleanly. LINT-010 runs only when a PRD has a Product Constants section. The checks from LINT-011 up are slim-shape only (they gate on the absence of a `## Technical Contract` heading): in full mode the Technical Contract legitimately holds wire taxonomy, so they never fire there.
