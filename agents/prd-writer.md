...
- **Nothing the user can notice lives only here**: a timeout, freshness window, retry limit, money format, or ordering that appears in a technical table must already have a Product Constants or Display Rules row. This section may repeat a value; it may never be its only home
