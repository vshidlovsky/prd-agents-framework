...
In `slim` mode the **Type column carries semantic types only** — `money amount`, `instant`, `string`, `boolean`, `enumeration`, `list of <entity>`, `error signal` — never units, epoch bases, or encodings ("number (minor units)", "number (epoch milliseconds)", "ISO-8601 string" are wire facts, dev-owned). Notes carry product semantics (what absent means, which affordance consumes the value) and point to the Display Rule that owns the rendering; the Display Rules worked examples (raw wire value → rendered output) are the sanctioned home for encoding knowledge. An encoding trap the team must not miss (a unit mismatch, an epoch base that differs from the rest of the product) goes in the canonical API reference entry for the endpoint, which the vocabulary row may cite.
...
- **Bounded behaviors name their episode, and the episode is defined once.** Any "at most once", "no more than N", or cooldown must state the episode it resets on, and that episode term must be defined a single time in the document and referenced everywhere else. Prefer user-initiated episodes (pressing retry, resubmitting) for anything the user can retry; lifecycle episodes (mount, page load, session) are for behaviors the user cannot re-trigger. A recovery budget tied to a lifecycle event while the user holds a Retry affordance produces a dead affordance after the first failure
- **Presentation determinants are user-perceivable and stay in the behavioral layer.** Timezone, currency and minor-unit handling, sort order, truncation policy, and rounding change what the customer sees, so they belong with the requirement they govern — a Display Rules row in the behavioral layer, never only a Technical Contract table, a display-formatting section, or a discrepancy row. The Technical Contract may repeat them; it must never be their sole home. A format name alone ("locale-aware short date", "formatted amount") is not a decision — state the determinant, and when the source value is already presentation-resolved (a plain date string, a preformatted amount), say so, since converting it would be a defect
...
| Budget scope | If this behavior is bounded ("at most once", "no more than N"), what episode is the bound tied to — a lifecycle event (mount, page load, session) or a user-initiated action (pressing retry, resubmitting)? If a recovery is bounded per lifecycle event but the user is offered an affordance that depends on it, the affordance becomes dead after the first failure. Bind recovery budgets to the user's action, and define the episode word once. |
...
2. **Every API endpoint verified** against API docs or code — explicitly marked with source, with request and response field lists reproduced verbatim from that source (exact names and casing, each field's required/optional status).
...
25. **Plain language — precise terms only where they earn their place.** Default to the plain word: page, button, link, screen, message. A term of art ("affordance", "surface", "presentation") is allowed only when it does distinguishing work in that sentence — e.g. "surface" genuinely covering entry point + screen collectively — and then define it once at first use. Write "the page doesn't exist", not "no referrer surface exists". And prefer the short sentence: if a sentence restates what its FR/AC reference already says, delete it. A designer or support agent must be able to read every requirement without translating it.
