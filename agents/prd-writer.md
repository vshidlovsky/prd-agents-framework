...
7. **Computed-example scan.** List every worked example containing numbers — Display Rules rows, currency conversions, date/time renderings, rounding or bucketing examples. For each, confirm the value was computed by executing the computation (Quality Standard #27) — you should be able to paste the verification command (the node/python one-liner with the target locale and timezone) for it. An example you cannot back with a command is composed, not computed: execute it now and replace the value with the command's output.
