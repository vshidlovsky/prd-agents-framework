...
2. **Behavioral/technical separation smells** (7 patterns): API field leaks, enum leaks, URL pattern leaks, UI copy in requirements, analytics event names inline, framework terminology, and design decisions in requirements. Design decisions include layout arrangements prescribing CSS structure and visual treatments — but NOT behavioral placement ("inline beneath input"), shipped DS component names, PM-decided display formats, "new browser tab", platform concepts, or generic UI vocabulary. Framework terminology includes library-specific terms but NOT CS jargon describing testable behavior (flag for rephrasing, not relocation) or product decisions/platform concepts that sound technical. See `rules/behavioral-separation.md` for the separation rules these patterns enforce.
