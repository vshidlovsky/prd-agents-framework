...
2. **Behavioral/technical separation smells** (7 patterns): API field leaks, enum leaks, URL pattern leaks, UI copy in requirements, analytics event names inline, framework terminology, and design decisions in requirements. Design decisions include any language that chooses how the UI renders something (component types, layout arrangements, visual treatments) rather than describing what the user sees or does. See `rules/behavioral-separation.md` for the separation rules these patterns enforce.
