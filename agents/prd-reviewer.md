...
| F-35 | Test coverage: every AC is bound to a verification approach, and any AC with no automated signal is named as manually verified with its trigger described. Behavioral-layer content — applies in `slim` and `full` mode alike | [PENDING] | [PENDING] |
...
| F-37 | Words used in this document: the block exists early in the PRD (in Context), and every recurring coined or borrowed term the document relies on has an entry with a one-sentence plain definition. Conformance, not advisory — applies in `slim` and `full` mode alike | [PENDING] | [PENDING] |
