...
| F-25 | Behavioral/Technical separation: FRs and ACs use semantic concept names with `[V#]` vocabulary markers — no API vocabulary, no CSS-level design decisions, no framework-specific terms. Allowed: shipped DS components, behavioral placement, PM-decided formats, "new browser tab", platform concepts, generic UI vocab, enum values in analytics ACs. CS jargon flagged for rephrasing only | [PENDING] | [PENDING] |
...
1. **Linguistic smells** (10 patterns): vague verbs, loopholes, ambiguous pronouns, passive voice, open-ended lists, superlatives, incomplete conditionals, subjective language, temporal comparisons, and implementation delegation.
2. **Behavioral/technical separation smells** (7 patterns): API field leaks, enum leaks, wire-detail leaks (paths, status codes, headers), UI copy/localization-key in requirements, analytics event names inline, framework terminology/implementation mechanism, and design decisions in requirements. These are detected by applying three generic tests (rename / designer-choice / QA-observability), not by matching against example lists. Design decisions include emphasis/variant qualifiers ("filled"/"tonal" button), layout, visual treatments, and over-specified ACs that enumerate screen furniture — but NOT behavioral placement ("inline beneath input"), shipped DS component names, PM-decided display formats, input/output mechanics that define the product, or generic UI nouns. Framework terminology/implementation mechanism includes library-specific terms and storage/transport internals but NOT CS jargon describing testable behavior (flag for rephrasing, not relocation) or product decisions that sound technical. Copy, localization keys, and translations are design-owned — flag any literal copy or key. See `rules/behavioral-separation.md`.

Smell detection is handled exclusively by **Matrix S** via a dedicated agent (Agent 5: Smell Reviewer). This separation ensures smell patterns get full attention — the agent reads each FR/AC against all 17 patterns with no other quality judgments competing. Matrix B and C do NOT check smells.
...
- Instruction: You are a quality reviewer focused on requirements clarity and behavioral/technical separation. Your job is to verify that FRs and ACs describe observable behavior without leaking implementation details or making design decisions. Read the smell patterns file cover to cover. Then for each FR and AC in the PRD: (1) Read the full text of the item. (2) Check against all 10 linguistic smell patterns. If a word or phrase matches a pattern, mark FAIL — quote the offending text and name the pattern. (3) Check against all 7 behavioral/technical separation smell patterns (see `rules/behavioral-separation.md`). Same standard — any match is FAIL. For the "design decision" pattern, apply this distinction: Generic UI vocabulary is ALLOWED — button, link, input, field, error message, page, screen, form, list, item, section, label, text, header, tab, menu, notification, dialog, alert, indicator, grid. Shipped DS component names (carousel, skeleton, bottom sheet) are ALLOWED when the PM explicitly chose them because the DS ships that component. Behavioral placement is ALLOWED — "inline beneath the input", "near the triggering element" describe where the user sees feedback; only flag placement that prescribes CSS positioning (sticky, fixed, absolute). "New browser tab" is ALLOWED — user-visible navigation. PM-decided display formats ("MM:SS", "0:30", "zero-padded") are ALLOWED — product requirements. Platform concepts ("browser's one-time-code autofill", "device identifier header") are ALLOWED. Enum values in analytics ACs are ALLOWED. Layout arrangements prescribing CSS structure are FAIL — sticky, full-surface, grid-3-column. Visual treatments are FAIL — pixel values, color variants, spacing tokens. Emphasis/variant qualifiers are FAIL — "primary, filled button", "tonal", "outlined" (a designer could choose differently — apply the designer-choice test). An AC that enumerates several visual elements or reads as a screen layout is FAIL — over-specification; an AC should assert one observable outcome. Any literal UI copy or localization-key path in an FR/AC is FAIL — copy, keys, and translations are design-owned, not PRD content (the only exception is wording mandated by law/compliance/contract with a cited source). Endpoint paths, raw HTTP status codes, and header names are FAIL — wire details that fail the rename test (map them in the Technical Contract). Implementation mechanism — where or how a value is stored or transported (secure-storage backend, keystore/keychain, transport protocol) — is FAIL by the QA-observability test; flag to relocate to TC, do not reword. For the "framework terminology" pattern, distinguish three tiers: (a) Library/framework-specific terms ("staleTime", "React Router state", "Zod schema") are FAIL — move to TC. (b) CS jargon describing testable behavior ("atomically replace", "first-expiry-wins", "invalidate the cached user profile") is FAIL for rephrasing — note "CS jargon — rephrase to QA-verifiable language, keep as FR" (do NOT suggest moving to TC). (c) Product decisions and platform concepts that sound technical ("replacing the current history entry", "proactive refresh", "browser's reported language") are NOT framework terminology — PASS. Do NOT suggest replacement text — just flag the problem and name the pattern. The writer should craft their own fix. (4) Only mark PASS if the text is genuinely clean against all 17 patterns. Work through one item at a time. Do NOT batch or skim.
